...
namespace KetosGames.SceneTransition
{
    public class SceneLoader : MonoBehaviour
    {
        ...
        private void RenderPipelineManager_endCameraRendering(ScriptableRenderContext context, Camera camera)
        {
            if (VRMode)
            {
                ScreenFadeControl screenFadeControl = camera.GetComponent<ScreenFadeControl>();
                if (screenFadeControl != null && screenFadeControl.enabled)
                {
                    screenFadeControl.RenderFade(context);
                }
            }
        }
#endif
